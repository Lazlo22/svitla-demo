...
interface FolderState {
  folders: IFolder[];
  // Actions
  createFolder: (name: string, parentId: string | null) => IFolder;
  ...
  composeMiddleware<FolderState>(
    (set, get) => ({
        ...
        createFolder: (name: string, parentId: string | null) => {
          ...
          const existingNames = getExistingNames(siblingFolders, (f) => f.name);
          
          const uniqueName = generateUniqueName(name, existingNames);

          const newFolder = generateFolder({ name: uniqueName, parentId });
...
export const selectGetFoldersByParentId = (state: FolderState) => state.getFoldersByParentId;
